...
for test_loader in test_loaders:
    ...
    for test_data in test_loader:
        ...
        mask = model.test()
        mask_density = (mask.sum()/mask.numel()).cpu().detach().numpy()
        test_results['mask_spars'].append(36.01 - 9.76 * (1 - mask_density))
    ...
    print(np.mean(test_results['mask_spars']), np.var(test_results['mask_spars']))
    print(np.mean(test_results['psnr_y']), np.std(test_results['psnr_y']))