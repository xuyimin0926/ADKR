...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        #self.cri_sprasity = nn.L1Loss().to(self.device)
        ...
        if self.is_train:
            ...
            # sparsity loss
            self.cri_sparsity = nn.L1Loss().to(self.device)
            self.l_sparsity_w = 1 #0.5 #0.2 #0.1 #0.05 #0.01 #1.5
            self.target_sparsity = 0.1 #0.7 #0.7 #  0.5
    ...
    def optimize_parameters(self, step, scaler):
        ...
        with autocast():
            l_all = 0
            self.fake_SR, self.fake_K, mask = self.netG(self.var_LN, self.real_K)
            ...
            # sparsity Loss
            """ if self.cri_sparsity is not None:
                l_sparsity = self.l_sparsity_w * (mask.sum()/mask.numel()-self.target_sparsity)**2
                l_all += l_sparsity
                self.log_dict['l_sparsity'] = l_sparsity.item() """
            ...
            if self.cri_sparsity is not None:
                try:
                    l_sparsity = self.l_sparsity_w * (mask.sum()/mask.numel() - self.target_sparsity) ** 2
                    l_all += l_sparsity
                    assert not torch.isnan(l_sparsity)
                except:
                    import IPython
                    IPython.embed()
                    assert 0
            ...
            self.log_dict['mask_den_train'] = (mask > 0).sum()/mask.numel()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.cal_lr_psnr:
                ...
                self.fake_LR = self.var_L
            try:
                self.log_dict['mask_den_test'] = (mask > 0).sum()/mask.numel()
            except:
                print('Density:{:.2f}'.format((mask > 0).sum()/mask.numel()))
